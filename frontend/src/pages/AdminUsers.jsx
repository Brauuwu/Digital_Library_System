...
export default function AdminUsers(){
  ...
  const submit = async (formData)=>{
    try{
      if(editing){ await api.put(`/users/${editing}`, formData); toast.push({ type:'success', message: 'Sửa người dùng thành công' }); }
      else{ await api.post('/users', formData); toast.push({ type:'success', message: 'Thêm người dùng thành công' }); }
      await load(); setEditing(null); setModalOpen(false);
    }catch(err){ toast.push({ type:'error', message: err.response?.data?.error || err.message }); }
  }
  const remove = async (id)=>{ if(!confirm('Xóa người dùng này?')) return; await api.delete(`/users/${id}`); toast.push({ type:'success', message: 'Đã xóa người dùng' }); load(); };
